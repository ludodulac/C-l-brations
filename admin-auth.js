(function(){
  ...
  window.getCelebrationsAdminToken=()=>sessionStorage.getItem(TOKEN_KEY)||'';
  ...
  function showGate(message=''){
    ...
    if(!gate){
      gate=document.createElement('div');gate.id='adminPasswordGate';gate.className='admin-password-gate';document.body.appendChild(gate);
    }
    gate.innerHTML=`<form class="admin-password-dialog" id="adminPasswordForm"><div class="eyebrow">Administration</div><h1>Accès administrateur</h1><p>Entrez le mot de passe administrateur pour accéder à la gestion du site.</p><label class="field"><span>Mot de passe</span><input id="adminGatePassword" type="password" autocomplete="current-password" autofocus></label><button class="btn primary" type="submit">Entrer</button>${message?`<div class="admin-auth-error">${esc(message)}</div>`:''}<a class="admin-gate-back" href="index.html">Retour au site</a></form>`;
    document.getElementById('adminPasswordForm').onsubmit=async e=>{
      ...
      sessionStorage.setItem(TOKEN_KEY,result.token);unlock();
    };
  }
