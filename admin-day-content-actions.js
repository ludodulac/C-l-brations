...
editDay=function(key){
  ...
  panel.innerHTML=`<div class="day-editor"><div class="admin-view-head" style="justify-content:flex-start"><button class="btn" onclick="editCelebration(${c.id})">Retour</button><div><h1>${esc(d.label)}</h1><div class="admin-subtle">${esc(c.archangel)} ${esc(c.year)}</div></div></div><div class="card day-editor-main"><div class="day-fields"><div class="form-grid">${dateFields}<label class="field full"><span>Texte</span><textarea id="dText">${esc(d.text||'')}</textarea></label></div><div id="dayLinks">${(d.links||[]).map((l,i)=>linkRow(l,i)).join('')}</div><div class="actions"><button type="button" class="btn small" id="addLink">+ Lien</button><button class="btn primary" id="saveDay">Enregistrer</button></div></div></div><div class="day-editor-columns"><div class="card"><div class="section-head"><h2>Contenus</h2><button type="button" class="btn small" id="associateDayContent">Associer un contenu existant</button></div>${dayLinkedContentRows(d)}${dayContentPicker(d)}</div><div class="card"><div class="section-head"><h2>Rendez-vous</h2></div><div id="dayEventForm"></div><div class="list">${events.length?events.map(eventRow).join(''):'<div class="notice">Aucun rendez-vous.</div>'}</div><div class="actions"><button class="btn small primary" onclick="showDayEventForm('${key}')">Ajouter un rendez-vous</button></div></div></div></div>`;
