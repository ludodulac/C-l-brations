...
function simpleLibraryCard(c){
  const visual=c.type==='Image'&&c.sourceType==='file'?`<img data-image-id="${c.id}" alt="${esc(c.name)}" class="library-thumb">`:c.hasCover?`<img data-cover-id="${c.id}" alt="Illustration de ${esc(c.name)}" class="library-thumb">`:window.contentFallbackVisual?window.contentFallbackVisual(c):'';
