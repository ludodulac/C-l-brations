...
function normalizeDay(d){const oldDate=d?.date||'';return{...d,key:d?.key||oldDate||dateKey(),label:d?.label||'Jour',startDate:d?.startDate||oldDate||'',endDate:d?.endDate||d?.startDate||oldDate||'',text:d?.text||'',links:Array.isArray(d?.links)?d.links.filter(x=>x&&x.url).map(x=>({label:x.label||x.url,url:x.url,kind:x.kind||'link'})):[],contentIds:Array.isArray(d?.contentIds)?d.contentIds.map(String):[]}}
